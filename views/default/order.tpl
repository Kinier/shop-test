    ...
    <tr>
        ...
        <td>
            <span id="itemCnt_{$item['id']}">
                <input type="hidden" name="itemCnt_{$item['id']}" value="{$item['cnt']}"> {*то есть это input, но возможности ввода нет*}
                {$item['cnt']}
            </span>
        </td>
        ...
        <td>
            <span id="itemPrice_{$item['id']}">
                <input type="hidden" name="itemPrice_{$item['id']}" value="{$item['price']}"> {*то есть это input, но возможности ввода нет*}
                {$item['price']}
            </span>
        </td>
        ...
        <td>
            <span id="itemRealPrice_{$item['id']}">
                <input type="hidden" name="itemRealPrice_{$item['id']}" value="{$item['realPrice']}"> {*то есть это input, но возможности ввода нет*}
                {$item['realPrice']}
            </span>
        </td>
    </tr>
...
{if isset($arUser)}
    ...
    <div id="orderUserInfoBox" {$buttonClass}>
        ...
        <table>
            ...
            <tr>
                <td>Адрес*</td>
                <td><textarea id="address" name="address">{$address}</textarea></td> {*value нету*}
            </tr>  
        </table>
    </div>
...
{else}
    ...
    <div id="registerBox"> или <br>
        <div class="menuCaption">Регистрация нового пользователя:</div>
            ...
            Адрес* :<input type="text" id="address" name="address" value=""/> <br>

            <input type="button" onclick="registerNewUser();" value="Зарегистрироватьься"/>
    </div>
